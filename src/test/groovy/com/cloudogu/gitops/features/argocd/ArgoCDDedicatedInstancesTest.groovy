...
class ArgoCDDedicatedInstancesTest extends ArgoCDTest {
    ...
    void 'GOP DedicatedInstances Central templating works correctly'() {
        ...
        assertThat(tenantProject['spec']['sourceRepos'][0 as String]).isEqualTo('http://scmm.scm-manager.svc.cluster.local/scm/repo/testPrefix-argocd/argocd')
        assertThat(tenantProject['spec']['sourceRepos'][1 as String]).isEqualTo('http://scmm.scm-manager.svc.cluster.local/scm/repo/testPrefix-argocd/cluster-resources')
    ...
    void 'Cluster Resource Misc templating'() {
        ...
        def miscYaml = new YamlSlurper().parse(Path.of argocdRepo.getAbsoluteLocalRepoTmpDir(), "/argocd/misc.yaml")
    ...
    void 'Append namespaces to Argocd argocd-default-cluster-config secrets'() {
        config.application.namespaces.dedicatedNamespaces = ['dedi-test1', 'dedi-test2', 'dedi-test3'].toSet()
        config.application.namespaces.tenantNamespaces = ['tenant-test1', 'tenant-test2', 'tenant-test3'].toSet()
        setup()
    ...
    void 'RBACs generated correctly'() {
        config.application.namespaces.tenantNamespaces = ['testprefix-tenant-test1', 'testprefix-tenant-test2', 'testprefix-tenant-test3'].toSet()
        setup()
        ...
        assertThat(rbacFolder.listFiles().count { it.isFile() }).isEqualTo(18)
