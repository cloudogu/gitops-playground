...
class ContentLoaderTest {
    ...
    void 'Combines content repos successfully'() {
        ...
        def repos = cloneContentRepos(createContent(config), config)
    ...
    void 'supports content variables'() {
        config.content.repos = [new ContentRepositorySchema(url: createContentRepo('folderBasedRepo1'), type: ContentRepoType.FOLDER_BASED, templating: true)]
    ...
    void 'Authenticates content Repos'() {
        config.content.repos = [new ContentRepositorySchema(url: createContentRepo('copyRepo1'), ref: 'main', type: ContentRepoType.COPY, target: 'common/repo', credentials: new Credentials('user', 'pw'))]
    ...
    void 'Checks out commit refs, tags and non-default branches for content repos'() {
        config.content.repos = [new ContentRepositorySchema(url: createContentRepo('', 'git-repository-with-branches-tags'), ref: 'someTag', type: ContentRepoType.COPY, target: 'common/tag'),
    ...
    void 'Checks out default branch when no ref set'() {
        config.content.repos = [new ContentRepositorySchema(url: createContentRepo('', 'git-repo-different-default-branch'), target: 'common/default', type: ContentRepoType.COPY),]
    ...
    void 'Respects order of folder-based repositories'() {
        config.content.repos = [// Note the different order!
    ...
    void 'Is able to COPY into MIRRORED repo'() {
        config.content.repos = [new ContentRepositorySchema(url: createContentRepo('mirrorRepo1', 'git-repository-with-branches-tags'), type: ContentRepoType.MIRROR, target: 'common/repo'),
    ...
    void 'Handles mirror and copy together'() {
        config.content.repos = [new ContentRepositorySchema(url: createContentRepo('folderBasedRepo1'), type: ContentRepoType.FOLDER_BASED),
                                new ContentRepositorySchema(url: createContentRepo('copyRepo2'), type: ContentRepoType.COPY, target: 'common/repo', overwriteMode: OverwriteMode.UPGRADE, path: 'subPath'),
    ...
    void 'Handles multiple mirrors of the same repo with different refs'() {
        ...
        config.content.repos = [new ContentRepositorySchema(url: repoToMirror, type: ContentRepoType.MIRROR, ref: 'main', target: 'common/repo'),
    ...
    void 'Handles targetRefs'() {
        config.content.repos = [// From branch to branch or tag to tag
    ...
    void 'Parses Repo coordinates'() {
        ...
        def actualTargetRepos = cloneContentRepos(content, config)
        ...
        expectedTargetRepos.each { expected ->
            ...
            def actual = actualTargetRepos.findAll { actual -> actual.namespace == expected.namespace && actual.repoName == expected.repoName
            }
            assertThat(actual).withFailMessage("Could not find repo with namespace=${expected.namespace} and repo=${expected.repoName} in ${actualTargetRepos}").hasSize(1)

            assertThat(actual[0].clonedContentRepo.absolutePath).isEqualTo(new File(findRoot(repos), "${expected.namespace}/${expected.repoName}").absolutePath)
        }
    }
    ...
    void 'Reset common repo to repo '() {
        ...
        config.content.repos = [new ContentRepositorySchema(url: createContentRepo('copyRepo1'), ref: 'main', type: ContentRepoType.COPY, target: 'common/repo'),
                                new ContentRepositorySchema(url: createContentRepo('copyRepo2'), type: ContentRepoType.COPY, target: 'common/repo', path: 'subPath')
        ...
        config.content.repos = [new ContentRepositorySchema(url: createContentRepo('copyRepo1'), ref: 'main', type: ContentRepoType.COPY, target: 'common/repo', overwriteMode: OverwriteMode.RESET),]
    ...
    void 'Update common repo test '() {
        ...
        config.content.repos = [new ContentRepositorySchema(url: createContentRepo('copyRepo2'), type: ContentRepoType.COPY, target: 'common/repo', path: 'subPath', overwriteMode: OverwriteMode.UPGRADE)]
    ...
    void 'init common repo, expect unchanged repo'() {
        ...
        config.content.repos = [new ContentRepositorySchema(url: createContentRepo('copyRepo1'), ref: 'main', type: ContentRepoType.COPY, target: 'common/repo', overwriteMode: OverwriteMode.INIT),]
    ...
    void 'ensure Jenkinsjob will be created'() {
         ...
         * Then again "RESET" to copyRepo1.
         * file content after that should be: copyRepo1*/
        config.content.repos = [new ContentRepositorySchema(url: createContentRepo('copyRepo1'), ref: 'main', type: ContentRepoType.COPY, createJenkinsJob: true, target: 'common/repo'),]
        scmmApiClient.mockRepoApiBehaviour()
        when(jenkins.isEnabled(any(DeploymentContext))).thenReturn(true)
    ...
    void 'ensure Jenkinsjob creation will be ignored'() {
         ...
         * Then again "RESET" to copyRepo1.
         * file content after that should be: copyRepo1*/
        config.content.repos = [new ContentRepositorySchema(url: createContentRepo('copyRepo1'), ref: 'main', type: ContentRepoType.COPY, createJenkinsJob: false, target: 'common/repo'),]
        scmmApiClient.mockRepoApiBehaviour()
        when(jenkins.isEnabled(any(DeploymentContext))).thenReturn(false)
        install(createContent(config), config)
    ...
    private List<RepoCoordinate> cloneContentRepos(ContentLoaderForTest contentLoader, Config config) {
        return contentLoader.cloneContentRepos(new ContextBuilder(config).build())
    }
