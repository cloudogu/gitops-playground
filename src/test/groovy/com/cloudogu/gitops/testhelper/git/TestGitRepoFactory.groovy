...
class TestGitRepoFactory extends GitRepoFactory {
    ...
    GitRepo create(String repoTarget, GitProvider scm) {
        ...
        if (!effectiveProvider) {
            throw new IllegalStateException("No GitProvider provided for repo '${repoTarget}' and defaultProvider is null.")
        }
        ...
        String prefixedRepoTarget = config.application.namePrefix + repoTarget
        GitRepo repoNew = new GitRepo(config, scm, prefixedRepoTarget, fileSystemUtils) {
            ...
            String getGitRepositoryUrl() {
                if (!remoteGitRepoUrl) {
                    ...
                    def originalRepo = System.getProperty("user.dir") + "/src/test/resources/com/cloudogu/gitops/utils/data/git-repository/"
