...
class InsecureCredentialProviderTest {
	...
	void 'ignores irrelevant items'() {
		...
		assertThat(provider.supports(new CredentialItem.InformationalMessage("This is not a relevant message"),
			new CredentialItem.YesNoType("This prompt is irrelevant as well"))).isFalse()
	}
	...
	void 'confirms insecure https processing'() {
		...
		assertThat(provider.supports(message,
			skipSingle,
			skipRepository,
			skipAlways)).isTrue()
