...
class GitopsPlaygroundCliTest {
    ...
    @Test
    void 'Runs config lifecycle hooks only for participating tools'() {
        AbstractTool regularTool = mock(AbstractTool)
        ContentLoader configLifecycleHook = mock(ContentLoader)
        when(application.getTools()).thenReturn([regularTool, configLifecycleHook])

        def status = cli.run('--yes')

        assertThat(status).isEqualTo(ReturnCode.SUCCESS)
        verify(configLifecycleHook).preConfigInit(any(Config))
        verify(configLifecycleHook).postConfigInit(any(Config))
    ...
    void 'ensure helm defaults are used, if not set'() {
        ...
        def fileConfig = [jenkins : [helm: [version: '5.8.1']],
                          scm     : [scmManager: [helm: [values: [initialDelaySeconds: 120]]]],
                          features: [monitoring : [helm: [version     : '66.2.1',
                                                          grafanaImage: 'localhost:30000/proxy/grafana:latest']],
                                     secrets    : [externalSecrets: [helm: [chart: 'my-secrets']],
        ...
        assertThat(cli.lastSchema.features.certManager.helm.version).isEqualTo('1.19.4')
