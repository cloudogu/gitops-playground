...
class K8sClientTest {
    ...
    @Test
    void 'createSecret updates an existing secret without deleting it'() {
    ...
    void 'createImagePullSecret creates docker registry secret'() {
        ...
        def requestBody = new JsonSlurper().parseText(server.getLastRequest().getUtf8Body()) as Map
        def dockerConfig = new JsonSlurper().parseText(requestBody["stringData"][".dockerconfigjson"] as String) as Map
        assertThat(dockerConfig["auths"]["docker.io"]["username"]).isEqualTo('user"name')
        assertThat(dockerConfig["auths"]["docker.io"]["password"]).isEqualTo('pa"ss')
    }
...
""")
    ...
    @Test
    void 'patch rejects an unknown patch type'() {
        def exception = shouldFail(IllegalArgumentException) {
            k8sApiClient.patch("pod", "test-pod", "default", "unknown", [:])
        }

        assertThat(exception.message).isEqualTo("Unsupported patch type: unknown")
    }
    ...
    @Test
    void 'delete without selectors removes all resources of the type'() {
        server.expect()
                .delete()
