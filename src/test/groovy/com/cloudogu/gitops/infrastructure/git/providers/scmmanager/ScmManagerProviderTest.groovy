...
class ScmManagerProviderTest {
	...
	private ScmManagerProvider newScmManager() {
		def scmManager = new ScmManagerProvider(new ContextBuilder(config).build(), scmmCfg, k8s, net, 'fv40-')
		scmManager.urls = urls
		scmManager.apiClient = apiClient
		return scmManager
	}
	...
	void 'createRepository returns true on 201 and false on subsequent 409 for the same repo'() {
		...
		when(repoApi.create(any(Repository), anyBoolean()))
			.thenAnswer(inv -> {
				...
				if (seen.contains(r.fullRepoName)) {
					return conflict
				}
	...
	void 'prometheusMetricsEndpoint is delegated to UrlResolver'() {
		...
		assertEquals(new URI('http://nodeport/scm/api/v2/metrics/prometheus'),
