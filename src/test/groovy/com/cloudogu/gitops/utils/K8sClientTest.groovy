...
class K8sClientTest {
	...
	void 'Gets internal nodeIp'() {
		...
		assertThat(commandExecutor.actualCommands[1]).isEqualTo("kubectl get node/k3d-gitops-playground-server-0 " +
	...
	void 'Creates secret'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo("kubectl create secret generic my-secret -n my-ns --from-literal key1=value1 --from-literal key2=value2" +
	...
	void 'get secret'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo("kubectl get secret my-secret -n my-ns -ojsonpath={.data.namespaces}")
	}
	...
	void 'Creates secret without namespace'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo("kubectl create secret generic my-secret --from-literal key1=value1 --dry-run=client -oyaml" +
			" | kubectl apply -f-")
	}
	...
	void 'Creates imagePullSecret without namespace'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo('kubectl create secret docker-registry my-reg' + ' --docker-server host --docker-username user --docker-password pw' +
			' --dry-run=client -oyaml | kubectl apply -f-')
	}
	...
	void 'Creates imagePullSecret'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo('kubectl create secret docker-registry my-reg -n my-ns' +
	...
	void 'Ensure in secret creation, nullable String become empty string'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo("kubectl create secret generic very-secret --from-literal isnullbecomeempty= --dry-run=client -oyaml" +
			" | kubectl apply -f-")
	}
	...
	void 'Creates configmap from file'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo("kubectl create configmap my-map -n my-ns --from-file /file --dry-run=client -oyaml" + " | kubectl apply -f-")
	}
	...
	void 'Creates configmap without namespace'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo("kubectl create configmap my-map --from-file /file --dry-run=client -oyaml" + " | kubectl apply -f-")
	}
	...
	void 'Creates service type nodePort'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo('kubectl create service nodeport my-svc -n my-ns --tcp 42:23 --node-port 32000' +
			' --dry-run=client -oyaml | kubectl apply -f-')
	}
	...
	void 'Creates service type nodePort without namespace and explicit nodePort'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo('kubectl create service nodeport my-svc --tcp 42:23' + ' --dry-run=client -oyaml | kubectl apply -f-')
	}
	...
	void 'Adds labels'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo("kubectl label secret my-secret -n my-ns --overwrite key1=value1 key2=value2")
	}
	...
	void 'Removes labels explicitly'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo("kubectl label node --all --overwrite key1- key2-")
	}
	...
	void 'Adds labels without namespace'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo("kubectl label secret my-secret --overwrite key1=value1 key2=value2")
	}
	...
	void 'Deletes'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo("kubectl delete secret -n my-ns --ignore-not-found=true --selector=key1=value1 --selector=key2=value2")
	}
	...
	void 'Deletes without namespace'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo("kubectl delete secret --ignore-not-found=true --selector=key1=value1 --selector=key2=value2")
	}
	...
	void 'Creates namespace when it does not exist'() {
		...
		assertThat(commandExecutor.actualCommands[1]).isEqualTo("kubectl create namespace my-ns")
	}
	...
	void 'Does not create namespace if it already exists'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo("kubectl get namespace my-ns")
	}
		...
		def serviceJson = '''
    {
		...
		assertThat(commandExecutor.actualCommands[1]).isEqualTo('kubectl patch service my-service -n my-namespace --type json -p [{"op":"replace","path":"/spec/ports/1/nodePort","value":32000}]')
	}
	...
	void 'Waits successfully until the resource reaches the desired phase'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo('kubectl get pod my-pod -n my-namespace -o jsonpath={.status.phase}')
	}
	...
	void 'Waits for node port of a service'() {
		...
		assertThat(commandExecutor.actualCommands[0]).isEqualTo('kubectl get service my-service -n my-namespace -o jsonpath={.spec.ports[0].nodePort}')
	...
	void 'Runs a pod with overrides'() {
		def overrides = [spec: [containers: [[name : "tmp-docker-gid-grepper",
		                                      image: "bash:5",]]]]
