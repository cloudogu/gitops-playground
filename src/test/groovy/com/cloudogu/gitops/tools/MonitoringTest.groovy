...
class MonitoringTest {
	...
	void 'is disabled via active flag'() {
		...
		assertFalse(createMonitoring().isEnabled(context))
	}
	...
	void 'is enabled via active flag'() {
		...
		assertThat(createMonitoring().isEnabled(context)).isTrue()
	}
	...
	void 'creates expected Helm deployment request'() {
		...
		assertThat(request.helmConfig)
			.isSameAs(config.features.monitoring.helm)

		assertThat(request.helmValuesPath)
			.isEqualTo(Monitoring.HELM_VALUES_PATH)
	...
	void 'passes additional Helm values through request'() {
		config.features.monitoring.helm.values = [key       : [some: 'thing',
		                                                       one : 1],
		...
		assertThat(request.helmConfig.values)
			.isEqualTo(config.features.monitoring.helm.values)
	}
	...
	void 'adds monitoring namespace and Grafana host to template data'() {
		...
		assertThat(request.templateData['monitoring']).isEqualTo([grafana: [host: 'grafana.local']])

		assertThat(request.templateData['namespaces']).isEqualTo(config.application.namespaces.activeNamespaces
	...
	void 'adds internal SCM Manager metrics endpoint to template data'() {
		...
		assertThat(request.templateData['scm']).isEqualTo([protocol: 'http',
		                                                   host    : 'localhost:8080',
		                                                   path    : '/scm/api/v2/metrics/prometheus'])
	}
	...
	void 'adds internal Jenkins metrics endpoint to template data'() {
		...
		assertThat(request.templateData['jenkins']).isEqualTo([metricsUsername: 'metrics',
		                                                       protocol       : 'http',
		                                                       host           : 'jenkins.foo-jenkins.svc.cluster.local',
		                                                       path           : '/prometheus'])
	}
	...
	void 'adds custom Jenkins metrics username to template data'() {
		...
		assertThat((request.templateData['jenkins'] as Map)
			['metricsUsername']).isEqualTo('external-metrics-username')
	}
	...
	void 'adds empty UID for Kubernetes deployment'() {
		...
		assertThat(request.templateData['uid']).isEqualTo('')
	}
	...
	void 'adds OpenShift UID to template data'() {
		...
		when(k8sClient.getAnnotation('namespace',
			'foo-monitoring',
			'openshift.io/sa.scc.uid-range')).thenReturn('1000920000/10000')

		HelmToolDeploymentRequest request =
			installAndCaptureRequest()

		assertThat(request.templateData['uid']).isEqualTo('1000920000')
	}
	...
	void 'creates Grafana mail secret when SMTP credentials are configured'() {
		...
		config.features.mail.smtpPassword = 'secret'
	...
	void 'applies Prometheus ServiceMonitor CRD from GitHub'() {
		...
		verify(k8sClient).applyYaml('https://raw.githubusercontent.com/' + 'prometheus-community/helm-charts/' +
	...
	void 'applies Prometheus ServiceMonitor CRD from local chart in air-gapped mode'() {
		...
		String expectedPath =
			rootChartsFolder.resolve('kube-prometheus-stack/' + 'charts/crds/crds/' + 'crd-servicemonitors.yaml').toString()

		install()

		verify(k8sClient).applyYaml(expectedPath)
	}
	...
	void 'prepares monitoring app content without copying templates'() {
		...
		assertThat(new File(clusterResourcesRepoDir,
			'apps/monitoring')).exists()

		assertThat(new File(clusterResourcesRepoDir,
			'apps/monitoring/templates')).doesNotExist()

		assertThat(new File(clusterResourcesRepoDir,
			'apps/monitoring/misc/dashboard')).exists()
	}
	...
	void 'removes dashboards for disabled features'() {
		...
		assertThat(new File(dashboardDirectory,
			'traefik-dashboard.yaml')).doesNotExist()

		assertThat(new File(dashboardDirectory,
			'traefik-dashboard-requests-handling.yaml')).doesNotExist()

		assertThat(new File(dashboardDirectory,
			'jenkins-dashboard.yaml')).doesNotExist()

		assertThat(new File(dashboardDirectory,
			'scmm-dashboard.yaml')).doesNotExist()
	}
	...
	void 'does not create namespace isolation RBAC resources by default'() {
		...
		assertThat(new File(clusterResourcesRepoDir,
			'apps/monitoring/misc/rbac')).doesNotExist()
	}
	...
	void 'publishes monitoring resources through repository workspace'() {
		...
		verify(repositoryWorkspace)
			.commitAndPushClusterResourcesChanges('Update monitoring GitOps resources')
	}
	...
	private boolean install() {
		...
		return createMonitoring().execute(deploymentContext,
			repositoryWorkspace)
	}

	private Monitoring createMonitoring() {
		return new Monitoring(helmToolDeployer,
			fileSystemUtils,
			k8sClient,
			gitHandler,
			imagePullSecretCreator)
		...
		TestGitRepoFactory repositoryFactory =
			new TestGitRepoFactory(config,
				...
				GitRepo create(String repositoryTarget,
					...
					createDashboardFiles(clusterResourcesRepoDir)
	...
	private static void createDashboardFiles(File repositoryDirectory) {
		File dashboardDirectory =
			new File(repositoryDirectory,
				'apps/monitoring/misc/dashboard')

		dashboardDirectory.mkdirs()

		new File(dashboardDirectory,
			'traefik-dashboard.yaml').text = 'dummy'

		new File(dashboardDirectory,
			'traefik-dashboard-requests-handling.yaml').text = 'dummy'

		new File(dashboardDirectory,
			'jenkins-dashboard.yaml').text = 'dummy'

		new File(dashboardDirectory,
			'scmm-dashboard.yaml').text = 'dummy'
	}

	private static Config createConfig() {
		return Config.fromMap(registry: [internal              : true,
