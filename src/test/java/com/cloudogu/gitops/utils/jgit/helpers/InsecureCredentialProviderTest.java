...
class InsecureCredentialProviderTest {
	...
	@Test
	void ignoresIrrelevantItems() {
		InsecureCredentialProvider provider = new InsecureCredentialProvider();
	...
	@Test
	void confirmsInsecureHttpsProcessing() throws UnsupportedCredentialItem, URISyntaxException {
		InsecureCredentialProvider provider = new InsecureCredentialProvider();

		CredentialItem.InformationalMessage message = new CredentialItem.InformationalMessage(
		...
		CredentialItem.YesNoType skipSingle = new CredentialItem.YesNoType(
			"Skip SSL verification for this single git operation");
		CredentialItem.YesNoType skipRepository = new CredentialItem.YesNoType(
			"Skip SSL verification for git operations for repository /tmp/groovy-generated-tmpdir-2746077697650757929/.git");
		CredentialItem.YesNoType skipAlways = new CredentialItem.YesNoType(
			"Always skip SSL verification for this server from now on");

		assertThat(provider.supports(message, skipSingle, skipRepository, skipAlways)).isTrue();

		assertThat(provider.get(
			new URIish("https://192.168.178.37/scm/repo/argocd/cluster-resources"),
			message,
			skipSingle,
			skipRepository,
			skipAlways))
		...
		assertThat(skipRepository.getValue()).isTrue();
		assertThat(skipAlways.getValue()).isFalse();
	}
}
