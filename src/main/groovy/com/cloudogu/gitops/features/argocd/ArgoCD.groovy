...
class ArgoCD extends Feature {
    ...
    protected final String central_scm_url_internal = "http://scmm-scm-manager.scm-manager.svc.cluster.local/scm"
    ...
    protected initRepos() {
        Boolean isCentralRepo = config.multiTenant.centralSCMUrl ? true : false

        argocdRepoInitializationAction = createRepoInitializationAction('argocd/argocd', 'argocd/argocd', isCentralRepo)

        clusterResourcesInitializationAction = createRepoInitializationAction('argocd/cluster-resources', 'argocd/cluster-resources', isCentralRepo)
