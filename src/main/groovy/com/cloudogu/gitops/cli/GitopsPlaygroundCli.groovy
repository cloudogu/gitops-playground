...
class GitopsPlaygroundCli {
    ...
    ReturnCode run(String[] args) {
        ...
        if (cliParams.application.outputConfigFile) {
    ...
    private Schema readConfigs(Schema cliParams) {
        ...
        def configPrecedence = [configMap, configFile, cliParams.toMap()]
        Map mergedConfigs = [:]
        configPrecedence.each {
            deepMerge(it, mergedConfigs)
        }
        
        Schema config = applicationConfigurator.initAndValidateConfig(mergedConfigs)
