...
public class ContentLoader extends Tool {
        ...
        for (ContentRepositorySchema repo : configToSet.getContent().getRepos()) {
            ...
            if (repo.getTarget() != null && !repo.getTarget().isEmpty()) {
                if (!repo.getTarget().contains("/")) {
                    throw new RuntimeException("content.target needs / to separate namespace/group from repo name. Repo: " + repo.getUrl());
                }
            }
            ...
            switch (repo.getType()) {
                case COPY:
                    if (repo.getTarget() == null || repo.getTarget().isEmpty()) {
                        throw new RuntimeException("content.repos.type " + ContentRepoType.COPY + " requires content.repos.target to be set. Repo: " + repo.getUrl());
                    }
                    break;
                case FOLDER_BASED:
                    if (repo.getTarget() != null && !repo.getTarget().isEmpty()) {
                        throw new RuntimeException("content.repos.type " + ContentRepoType.FOLDER_BASED + " does not support target parameter. Repo: " + repo.getUrl());
                    }
                    if (repo.getTargetRef() != null && !repo.getTargetRef().isEmpty()) {
                        throw new RuntimeException("content.repos.type " + ContentRepoType.FOLDER_BASED + " does not support targetRef parameter. Repo: " + repo.getUrl());
                    }
                    break;
                case MIRROR:
                    if (repo.getTarget() == null || repo.getTarget().isEmpty()) {
                        throw new RuntimeException("content.repos.type " + ContentRepoType.MIRROR + " requires content.repos.target to be set. Repo: " + repo.getUrl());
                    }
                    if (!ContentRepositorySchema.DEFAULT_PATH.equals(repo.getPath())) {
                        throw new RuntimeException("content.repos.type " + ContentRepoType.MIRROR + " does not support path. Current path: " + repo.getPath() + ". Repo: " + repo.getUrl());
                    }
                    if (Boolean.TRUE.equals(repo.getTemplating())) {
                        throw new RuntimeException("content.repos.type " + ContentRepoType.MIRROR + " does not support templating. Repo: " + repo.getUrl());
                    }
                    break;
            }
        }
    ...
    protected void deployHelmReleasesFromContent() throws Exception {
        if (getConfig().getContent() == null || getConfig().getContent().getHelmReleases() == null || getConfig().getContent().getHelmReleases().isEmpty()) {
        ...
        for (var helmRelease : getConfig().getContent().getHelmReleases()) {
            ...
            String releaseName = (helmRelease.getReleaseName() != null && !helmRelease.getReleaseName().isEmpty())
                    ? helmRelease.getReleaseName()
                    : helmRelease.getName();

            deployHelmChart(helmRelease.getName(),
                    releaseName,
    ...
    void createImagePullSecrets() {
        if (Boolean.TRUE.equals(getConfig().getRegistry().getCreateImagePullSecrets())) {
            String registryUsername = (getConfig().getRegistry().getReadOnlyUsername() != null && !getConfig().getRegistry().getReadOnlyUsername().isEmpty())
                    ? getConfig().getRegistry().getReadOnlyUsername()
                    : getConfig().getRegistry().getUsername();

            String registryPassword = (getConfig().getRegistry().getReadOnlyPassword() != null && !getConfig().getRegistry().getReadOnlyPassword().isEmpty())
    ...
    private void createRepoCoordinates(ContentRepositorySchema repoConfig, File mergedReposFolder, List<RepoCoordinate> repoCoordinates) throws Exception {
        ...
        log.debug("Cloning content repo, {}, revision {}, path {}, overwriteMode {}",
                repoConfig.getUrl(), repoConfig.getRef(), repoConfig.getPath(), repoConfig.getOverwriteMode());

        if (repoConfig.getCredentials() != null && repoConfig.getCredentials().getUsername() != null && repoConfig.getCredentials().getPassword() != null) {
            credentialsProvider = new UsernamePasswordCredentialsProvider(repoConfig.getCredentials().getUsername(), repoConfig.getCredentials().getPassword());
        } else if (repoConfig.getCredentials() != null && repoConfig.getCredentials().getSecretName() != null && repoConfig.getCredentials().getSecretNamespace() != null) {
    ...
    private static void createRepoCoordinatesForTypeCopy(ContentRepositorySchema repoConfig, File contentRepoDir, File mergedRepoFolder, File repoTmpDir,
    ...
    private static void createRepoCoordinatesForTypeFolderBased(ContentRepositorySchema repoConfig, File repoTmpDir, File contentRepoDir, File mergedRepoFolder,
                                                                List<RepoCoordinate> repoCoordinates) {
            ...
            try {
                engine.replaceTemplates(srcPath, Map.of(
                        ...
                        "statics", !getConfig().getContent().getUseWhitelist()
                                ...
                                : new AllowListFreemarkerObjectWrapper(Configuration.VERSION_2_3_32, getConfig().getContent().getAllowedStaticsWhitelist()).getStaticModels()
                ));
            } catch (Exception e) {
    ...
    private void cloneToLocalFolder(ContentRepositorySchema repoConfig, File repoTmpDir) {
        CloneCommand cloneCommand = gitClone()
    ...
    private static String findRef(ContentRepositorySchema repoConfig, Repository gitRepo) {
        try {
            ...
            var remoteCommand = Git.lsRemoteRepository()
            ...
            for (Ref ref : refs) {
                if (ref.getName().endsWith(repoConfig.getRef())) {
            ...
            if (potentialRef == null) {
                throw new RuntimeException("Reference '" + repoConfig.getRef() + "' not found in content repository '" + repoConfig.getUrl() + "'");
            }
    ...
    private static void handleRepoCopyingOrFolderBased(RepoCoordinate repoCoordinate, GitRepo targetRepo, boolean isNewRepo) throws Exception {
        ...
        String targetRefShort = repoCoordinate.repoConfig.getTargetRef().replace("refs/heads/", "").replace("refs/tags/", "");
    ...
    private static String setRefSpec(RepoCoordinate repoCoordinate, String targetRefShort) {
        String refSpec;
        if ((repoCoordinate.refIsTag && !repoCoordinate.repoConfig.getTargetRef().startsWith("refs/heads")) || repoCoordinate.repoConfig.getTargetRef().startsWith("refs/tags")) {
    ...
    private static void clearTargetRepoIfApplicable(RepoCoordinate repoCoordinate, GitRepo targetRepo) {
        if (OverwriteMode.INIT != repoCoordinate.repoConfig.getOverwriteMode()) {
            if (OverwriteMode.RESET == repoCoordinate.repoConfig.getOverwriteMode()) {
                log.info("OverwriteMode " + OverwriteMode.RESET +
                        " set for repo '" +
                        repoCoordinate.getFullRepoName() +
                        "': " +
                ...
                log.debug("OverwriteMode " + OverwriteMode.UPGRADE +
                        " set for repo '" +
                        repoCoordinate.getFullRepoName() +
                        "': " +
    ...
    private static void handleRepoMirroring(RepoCoordinate repoCoordinate, GitRepo targetRepo) throws Exception {
        ...
        if (repoCoordinate.repoConfig.getRef() != null && !repoCoordinate.repoConfig.getRef().isEmpty()) {
            ...
            if (repoCoordinate.repoConfig.getTargetRef() != null && !repoCoordinate.repoConfig.getTargetRef().isEmpty()) {
                log.debug("Mirroring repo '{}' ref '{}' to target repo {}, targetRef: '{}'",
                        repoCoordinate.repoConfig.getUrl(), repoCoordinate.repoConfig.getRef(), repoCoordinate.getFullRepoName(), repoCoordinate.repoConfig.getTargetRef());
                targetRepo.pushRef(repoCoordinate.repoConfig.getRef(), repoCoordinate.repoConfig.getTargetRef(), true);
            } else {
                log.debug("Mirroring repo '{}' ref '{}' to target repo {}",
                        repoCoordinate.repoConfig.getUrl(), repoCoordinate.repoConfig.getRef(), repoCoordinate.getFullRepoName());
    ...
    private static void validateCommitReferences(RepoCoordinate repoCoordinate) {
        if (GitRepo.isCommit(repoCoordinate.clonedContentRepo, repoCoordinate.repoConfig.getRef())) {
            throw new RuntimeException("Mirroring commit references is not supported for content repos at the moment. content repository '" +
                    repoCoordinate.repoConfig.getUrl() + "', ref: " + repoCoordinate.repoConfig.getRef());
        }
    }
    ...
    static boolean isValidForPush(boolean isNewRepo, RepoCoordinate repoCoordinate) {
        if (!isNewRepo && OverwriteMode.INIT == repoCoordinate.repoConfig.getOverwriteMode()) {
            log.warn("OverwriteMode " + OverwriteMode.INIT +
                    " set for repo '" +
                    repoCoordinate.getFullRepoName() +
    ...
    public static class RepoCoordinate {
        ...
        public RepoCoordinate findSameNotMirror(List<RepoCoordinate> repoCoordinates) {
            return repoCoordinates.stream()
                    .filter(it -> it.getFullRepoName().equals(getFullRepoName()) && ContentRepoType.MIRROR != it.repoConfig.getType())
