...
public class RepositoryProvisioning {
    ...
    public void publishClusterResourcesAndTenantBootstrapRepositoryChanges(String toolName, String message) {
        assertWorkspacePrepared();
        String actualMessage = message != null ? message : "Update " + toolName + " resources";
        try {
            workspace.commitAndPushClusterResourcesAndTenantBootstrapChanges(actualMessage);
        } catch (Exception e) {
            throw new RuntimeException("Failed to publish cluster resources and tenant bootstrap repository changes", e);
        }
    }
