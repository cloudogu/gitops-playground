...
public void generate() {
	...
	throw new IllegalStateException(
		"SCMM repo must be set using withRepo() before calling generate()");
	}

	log.trace(
		"Generating RBAC for name='{}', namespace='{}', subfolder='{}'",
		name,
		namespace,
		subfolder);
...
private void generateRole(File outputDir) {
	...
	try {
	templater.template(
		role.getTemplateFile(), role.getOutputFile(outputDir), role.toTemplateParams());
	} catch (Exception e) {
...
private void generateRoleBinding(File outputDir) {
	...
	try {
	templater.template(
		binding.getTemplateFile(), binding.getOutputFile(outputDir), binding.toTemplateParams());
	} catch (Exception e) {
