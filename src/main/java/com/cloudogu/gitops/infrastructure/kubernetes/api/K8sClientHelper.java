...
class K8sClientHelper {
  ...
  static String waitForPodCompletion(
    ...
    while (tryCount < defaultRetries) {
      ...
      tryCount++;
      log.debug(
          "Still waiting for pod/{} to complete... (try {}/{})", podName, tryCount, defaultRetries);
      try {
  ...
  static void validateNamespaceName(String name) {
    if (name == null || name.trim().isEmpty()) {
      throw new IllegalArgumentException(
          "Namespace name must be provided and cannot be null or empty.");
    }
  }
  ...
  static io.fabric8.kubernetes.client.dsl.Resource<HasMetadata> getResourceClient(
    ...
    return (io.fabric8.kubernetes.client.dsl.Resource<HasMetadata>)
        resolveResourceClient(client, resourceType, name, resolvedNamespace);
  }
  ...
  private static ResourceDefinitionContext toResourceDefinitionContext(Map<String, Object> match) {
    return new ResourceDefinitionContext.Builder()
        .withGroup((String) match.get(GROUP_KEY))
  ...
  private static Map<String, Object> findApiResourceInGroup(
      KubernetesClient client, APIGroup group, String normalized, String original) {
    for (String version : groupVersions(group)) {
      APIResource resolved =
          findMatchingResourceInVersion(client, group, version, normalized, original);
      if (resolved != null) {
  ...
  private static APIResource findMatchingResourceInVersion(
      KubernetesClient client, APIGroup group, String version, String normalized, String original) {
  ...
  private static List<APIResource> fetchApiResources(
      KubernetesClient client, APIGroup group, String version) {
    try {
  ...
  private static boolean matchesResource(APIResource res, String normalized, String original) {
    boolean match =
        res.getKind().equalsIgnoreCase(original)
            || res.getName().equalsIgnoreCase(normalized)
            || (res.getSingularName() != null
                && res.getSingularName().equalsIgnoreCase(normalized));
  ...
  private static Map<String, Object> toResourceMatch(
      APIGroup group, String version, APIResource resolved) {
