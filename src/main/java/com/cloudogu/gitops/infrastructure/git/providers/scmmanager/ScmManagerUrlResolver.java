...
public class ScmManagerUrlResolver {
	...
	public String inClusterRepoUrl(String repoTarget) {
		...
		return noTrailSlash(withSlash(inClusterBase()).resolve(REPO_ROOT + "/" + repo + "/")).toString();
	}
	...
	private URI serviceDnsBase() {
		return URI.create(HTTP_PREFIX + serviceName() + "." + serviceNamespace() + ".svc.cluster.local");
	}
