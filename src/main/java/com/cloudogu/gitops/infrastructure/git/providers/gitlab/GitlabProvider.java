...
public GitlabProvider(DeploymentContext context, GitlabConfig gitlabConfig) {
	...
	String url =
		Objects.requireNonNull(gitlabConfig.getUrl(), "Missing gitlab url in config.scm.gitlab.url")
			.trim();
...
@Override
public void setRepositoryPermission(
	String repoTarget, String principal, AccessRole role, Scope scope) {
...
private long ensureSubgroupUnderParentId(Group parent, String segPath) {
	...
	throw new IllegalStateException(
		"Cannot create subgroup '"
			+ segPath
			+ "' under '"
			+ parent.getFullPath()
			+ "': "
			+ "a project with that path already exists at '"
			+ parent.getFullPath()
			+ "/"
			+ segPath
			+ "'. "
