...
public class ClusterResourcesCopyFilter {
    ...
    public static FileFilter forSubDirs(String copyFromDirectory, Collection<String> subDirsToCopy) {
        ...
        Set<String> prefixes = subDirsToCopy.stream().map(s -> {
        ...
        return f -> {
            ...
            if (templateIncludePrefixes.stream().anyMatch(p -> (isDir ? relDir : finalRel).startsWith(p))) {
                return true;
            }
            ...
            if (isDir) {
                return prefixes.stream().anyMatch(p -> relDir.equals(p) || relDir.startsWith(p) || p.startsWith(relDir));
            }
