...
public class Registry extends Tool {
    ...
    protected String activeNamespace(DeploymentContext context) {
        return Boolean.TRUE.equals(context.getConfig().getRegistry().getInternal())
                ? context.getConfig().getApplication().getNamePrefix() + context.getConfig().getRegistry().getNamespace()
                : null;
    }
    ...
    private void deployInternalRegistry() {
        deployHelmChart(TOOL_NAME,
                RELEASE_NAME,
                namespace,
                context.getConfig().getRegistry().getHelm(),
                "",
                context,
                true);
    }
    ...
    private void createInternalRegistryNodePortIfRequired() {
        ...
        k8sClient.createServiceNodePort("docker-registry-internal-port",
                ...
                context.getConfig().getRegistry().getInternalPort().toString(),
                namespace);
    }
