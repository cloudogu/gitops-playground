...
public class Vault extends Tool {
    ...
    protected String activeNamespace(DeploymentContext context) {
        return context.getConfig().getApplication().getNamePrefix() + context.getConfig().getFeatures().getSecrets().getNamespace();
    }
    ...
    private void prepareDevModeIfRequired() {
        ...
        try {
            postStartScript = new TemplatingEngine().replaceTemplate(templatedFile.toFile(), Map.of("namePrefix", getConfig().getApplication().getNamePrefix()));
        } catch (Exception e) {
        ...
        addHelmValuesData("dev", Map.of(
                "rootToken", UUID.randomUUID().toString(),
                "vaultPostStartConfigMap", vaultPostStartConfigMap,
                "vaultPostStartVolume", vaultPostStartVolume,
                "postStartScriptName", postStartScript.getName()
        ));
    }
