  ...
  public void createIfRequired(Config config, String namespace) {
    ...
    if (namespace == null || namespace.isEmpty()) {
      throw new IllegalArgumentException(
          "Namespace must be set before creating an image pull secret.");
    }
    ...
    String user =
        firstNonBlank(
            ...
            firstNonBlank(
                config.getRegistry().getReadOnlyUsername(), config.getRegistry().getUsername()));
    String password =
        firstNonBlank(
            config.getRegistry().getProxyPassword(),
            firstNonBlank(
                config.getRegistry().getReadOnlyPassword(), config.getRegistry().getPassword()));
