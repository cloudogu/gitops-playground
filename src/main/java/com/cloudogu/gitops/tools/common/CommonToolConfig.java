...
public class CommonToolConfig extends Tool {
     ...
     * Throws RuntimeException with meaningful message, if invalid.
     */
    ...
    private void validateMirrorReposHelmChartFolderSet(Config configToSet) {
        if (Boolean.TRUE.equals(configToSet.getApplication().getMirrorRepos()) &&
                (configToSet.getApplication().getLocalHelmChartFolder() == null || configToSet.getApplication().getLocalHelmChartFolder().isEmpty())) {
