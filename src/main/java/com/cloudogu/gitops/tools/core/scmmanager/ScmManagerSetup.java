...
public class ScmManagerSetup {
    ...
    public void setupHelm() {
        ...
        deployer.getHelmStrategy().deployFeature(helmConfig.getRepoURL(),
                "scm-manager",
                helmConfig.getChart(),
    ...
    public void createArgocdApplication() {
        ...
        deployer.deployFeature(helmConfig.getRepoURL(),
                "scm-manager",
                helmConfig.getChart(),
                helmConfig.getVersion(),
                this.scmManager.getScmmConfig().getNamespace(),
    ...
    private Path prepareHelmValues() {
        ...
        try {
            TemplateModel statics = new DefaultObjectWrapperBuilder(Configuration.VERSION_2_3_32).build().getStaticModels();
        ...
        Map<String, Object> values = this.scmManager.getScmmConfig().getHelm().getValues() != null ? 
                this.scmManager.getScmmConfig().getHelm().getValues() : new HashMap<>();
    ...
    private void installScmmPlugins() {
        ...
        for (int i = 0; i < pluginNames.size(); i++) {
            ...
            restartForThisPlugin = !this.scmManager.getConfig().getScm().getScmManager().getSkipRestart() && i == pluginNames.size() - 1;
    ...
    private void addDefaultUsers() {
        ...
        addUser(this.scmManager.getScmmConfig().getGitOpsUsername(), this.scmManager.getScmmConfig().getPassword(), "changeme@test.local");
