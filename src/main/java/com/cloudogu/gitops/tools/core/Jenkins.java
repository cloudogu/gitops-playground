...
public class Jenkins extends Tool {
  ...
  private static final List<String> OIDC_BOOT_PLUGIN_NAMES =
      Arrays.asList("oic-auth", "json-path-api");
  ...
  private void runSetupScript() {
    ...
    if (getConfig().getJenkins().getAdditionalEnvs() != null) {
      for (Map.Entry<String, String> entry :
          getConfig().getJenkins().getAdditionalEnvs().entrySet()) {
    ...
    if (userManager.isUsingSecurityRealmWithoutLocalUserCreation()) {
      ...
      userManager.createUser(
          ...
          getConfig().getJenkins().getMetricsPassword());
    }
  ...
  public void createJenkinsjob(String namespace, String repoName) {
    ...
    jobManager.createJob(
        jobName, this.gitHandler.getTenant().getUrl(), prefixedNamespace, credentialId);
  ...
  private boolean jenkinsOidcConfigured() {
    return getConfig().getJenkins().getOidc() != null
        && !getConfig().getJenkins().getOidc().trim().isEmpty();
  }

  private List<String> getJenkinsOidcBootPlugins() {
    File pluginsFile =
        new File(fileSystemUtils.getRootDir() + "/scripts/jenkins/plugins/plugins.txt");
    ...
    List<String> missingPlugins =
        OIDC_BOOT_PLUGIN_NAMES.stream().filter(name -> !pinnedPlugins.containsKey(name)).toList();

    if (!missingPlugins.isEmpty()) {
      throw new IllegalStateException(
          "Required Jenkins OIDC boot plugins missing from "
              + pluginsFile
              + ": "
