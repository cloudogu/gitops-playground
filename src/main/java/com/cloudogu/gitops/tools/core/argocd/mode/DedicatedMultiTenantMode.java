...
public class DedicatedMultiTenantMode implements DeploymentMode {
    ...
    public void createSCMCredentialsSecret() {
        log.debug("Creating tenant repo credential secret that is used by tenant ArgoCD to access repos in {}", config.getScm().getScmProviderType());
        ...
        log.debug("Creating central repo credential secret that is used by central ArgoCD to access repos in {}", config.getScm().getScmProviderType());
    ...
    public void updateManagedNamespaces() {
        ...
        k8sClient.patch("secret",
                ...
                namespace,
                Map.of("stringData", Map.of("namespaces", String.join(",", config.getApplication().getNamespaces().getTenantNamespaces()))));
    ...
    private void generateCentralArgoCDRBAC() {
        for (String ns : config.getApplication().getNamespaces().getActiveNamespaces()) {
            ...
            new RbacDefinition(Role.Variant.ARGOCD)
                    ...
                    .withNamespace(ns)
                    .withServiceAccountsFrom(config.getMultiTenant().getCentralArgocdNamespace(), ARGOCD_SERVICE_ACCOUNTS)
                    .withConfig(config)
                    .withRepo(repositoryWorkspace.getClusterResourcesRepository())
    ...
    private void updateCentralManagedNamespaces() {
        String base64Namespaces = (String) k8sClient.getArgoCDNamespacesSecret("argocd-default-cluster-config",
                config.getMultiTenant().getCentralArgocdNamespace());
