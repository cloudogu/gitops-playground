...
public class ArgoCDRepoSetup {
  ...
  private static final String TENANT_BOOTSTRAP_SOURCE_DIR =
  ...
  public ArgoCDRepoLayout tenantRepoLayout() {
    if (!repositoryWorkspace.hasTenantBootstrapRepository()) {
      throw new IllegalStateException(
          "tenantBootstrap repo is not initialized in single-instance mode.");
    }
  ...
  private void validateRepositoryWorkspace() {
    ...
    try {
      String clusterRoot =
          new File(repositoryWorkspace.clusterResourcesRootDir()).getCanonicalPath();
    ...
    if (context.isMultiTenant()) {
      log.debug(
          "Deleting unnecessary non dedicated instances folders from argocd repo: "
              + "applications={}, projects={}, tenant={}/tenant",
  ...
  private Map<String, Object> buildTemplateValues(GitRepo repo) {
    ...
    try {
      argocd.put(
          "host",
          getConfig().getFeatures().getArgocd().getUrl() != null
                  && !getConfig().getFeatures().getArgocd().getUrl().isEmpty()
    ...
    scm.put(
        "centralScmUrl",
        gitHandler.getCentral() != null ? gitHandler.getCentral().repoPrefix() : "");
    values.put("scm", scm);
