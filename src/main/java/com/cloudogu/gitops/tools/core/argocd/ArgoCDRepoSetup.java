...
public class ArgoCDRepoSetup {
    ...
    private void prepareClusterResourcesLayout() {
        ...
        if (context.isMultiTenant()) {
            log.debug("Deleting unnecessary non dedicated instances folders from argocd repo: " +
                    "applications={}, projects={}, tenant={}/tenant", layout.applicationsDir(), layout.projectsDir(), layout.multiTenantDir());
    ...
    private Map<String, Object> buildTemplateValues(GitRepo repo) {
        ...
        try {
            argocd.put("host", getConfig().getFeatures().getArgocd().getUrl() != null && !getConfig().getFeatures().getArgocd().getUrl().isEmpty() ? new URL(getConfig().getFeatures().getArgocd().getUrl()).getHost() : "");
