...
public static final String SCM_PROVIDER_TYPE_DESCRIPTION =
	"The SCM provider type. Possible values: SCM_MANAGER, GITLAB";
...
public static class GitlabTenantConfig implements GitlabConfig {
	...
	public static final String GITLAB_INTERNAL_DESCRIPTION =
		"True if Gitlab is running in the same K8s cluster. For now we only support access by external URL";
	public static final String GITLAB_URL_DESCRIPTION = "Base URL for the Gitlab instance";
	public static final String GITLAB_USERNAME_DESCRIPTION =
		"Defaults to: oauth2.0 when PAT token is given.";
	public static final String GITLAB_TOKEN_DESCRIPTION =
		"PAT Token for the account. Needs read/write repo permissions. See docs for mor information";
	public static final String GITLAB_PARENT_GROUP_ID =
		"Number for the Gitlab Group where the repos and subgroups should be created";
	...
	@Option(
		names = {"--gitlab-username"},
		description = GITLAB_USERNAME_DESCRIPTION)
	@JsonPropertyDescription(GITLAB_USERNAME_DESCRIPTION)
	private String username = "oauth2.0";

	@Option(
		names = {"--gitlab-token"},
		description = GITLAB_TOKEN_DESCRIPTION)
	@JsonPropertyDescription(GITLAB_TOKEN_DESCRIPTION)
	private String password;

	@Option(
		names = {"--gitlab-group-id"},
		description = GITLAB_PARENT_GROUP_ID)
...
public static class ScmManagerTenantConfig implements ScmManagerConfig {
	...
	public static final String SCMM_SKIP_RESTART_DESCRIPTION =
		"Skips restarting SCM-Manager after plugin installation. Use with caution! If the plugins are not installed up front, the installation will likely fail. The intended use case for this is after the first installation, for config changes only. Do not use on first installation or upgrades.'";
	public static final String SCMM_SKIP_PLUGINS_DESCRIPTION =
	...
	public static final String SCMM_NAMESPACE_DESCRIPTION =
		"Namespace where SCM-Manager should run";
	...
	@Option(
		names = {"--scmm-skip-restart"},
		description = SCMM_SKIP_RESTART_DESCRIPTION)
	@JsonPropertyDescription(SCMM_SKIP_RESTART_DESCRIPTION)
	private Boolean skipRestart = false;

	@Option(
		names = {"--scmm-skip-plugins"},
		description = SCMM_SKIP_PLUGINS_DESCRIPTION)
