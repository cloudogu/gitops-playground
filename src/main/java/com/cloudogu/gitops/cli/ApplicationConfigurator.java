...
public class ApplicationConfigurator {
  ...
  private void addScmConfig(Config newConfig) {
    ...
    if (hasText(newConfig.getScm().getScmManager().getUrl())) {
      ...
      newConfig
          .getScm()
          .getScmManager()
          .setUrlForJenkins(newConfig.getScm().getScmManager().getUrl());
    } else {
      ...
      newConfig
          ...
          .setUrlForJenkins(
              "http://scmm."
                  + newConfig.getApplication().getNamePrefix()
                  + newConfig.getScm().getScmManager().getNamespace()
                  + ".svc.cluster.local/scm");
    }
  ...
  public boolean isUrlSetAndValid(Config config) {
    ...
    if (hasText(url)) {
      try {
        log.debug(
            "Validating user-provided features.argocd.resourceInclusionsCluster URL: {}", url);
        new URL(url);
        log.info("Found valid URL in features.argocd.resourceInclusionsCluster: {}", url);
        return true;
      } catch (MalformedURLException e) {
        throw new IllegalArgumentException(
            "Invalid URL for 'features.argocd.resourceInclusionsCluster': " + url + ".", e);
      }
    }
    return false;
  }
  ...
  public void buildAndValidateURLFromEnvironment(Config config) {
    log.debug(
        "Attempting to set features.argocd.resourceInclusionsCluster via Kubernetes ENV variables.");
