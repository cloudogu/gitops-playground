...
public class GenerateJsonSchema {
    ...
    public static String generateDocs() {
        ...
        for (Field f : topFields) {
            md.append("- [").append(sectionTitle(f.getName())).append("](#").append(anchor(f.getName())).append(")\n");
        }
        md.append("- [Tools](#tools)\n");
        for (Field f : schemaFields(Config.FeaturesSchema.class)) {
            md.append("  - [").append(sectionTitle(f.getName())).append("](#tools-").append(anchor(f.getName())).append(")\n");
        }
        md.append("\n");
        ...
        for (Field field : schemaFields(Config.FeaturesSchema.class)) {
            ...
            try {
                md.append(buildTable(field.get(config.getFeatures()), field.getType(), "features." + field.getName()));
            } catch (IllegalAccessException e) {
                throw new RuntimeException(e);
            }
        }
    ...
    public static String buildTable(Object instance, Class<?> clazz, String prefix) {
        ...
        for (Map<String, String> r : rows) {
            sb.append("| ").append(r.get("cli"))
                    .append(" | `").append(r.get("key"))
                    .append("` | ").append(r.get("type"))
                    .append(" | `").append(r.get("default"))
                    .append("` | ").append(r.get("desc"))
                    .append(" |\n");
        }
        sb.append("\n");
        return sb.toString();
    ...
    public static List<Map<String, String>> collectRows(Object instance, Class<?> clazz, String prefix) {
        ...
        for (Field field : allFields(clazz)) {
            ...
            if (isSchemaType(field.getType())) {
                ...
                if (cliOpt != null) {
                    r.put("cli", Arrays.stream(cliOpt.names()).map(opt -> "`" + opt + "`").collect(Collectors.joining(", ")));
                } else {
                    r.put("cli", "-");
                }
                r.put("key", key);
                ...
                r.put("desc", (jsonDesc != null ? jsonDesc.value() : "-").replaceAll("\\s*\\n\\s*", " ").trim());
                rows.add(r);
            }
        }
        return rows;
