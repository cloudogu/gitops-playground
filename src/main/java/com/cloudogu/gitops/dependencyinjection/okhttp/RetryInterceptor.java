...
public class RetryInterceptor implements Interceptor {
  ...
  private static final Set<Integer> STATUS_CODES_TO_RETRY =
      Set.of(
          408, // Request Timeout
  ...
  public Response intercept(@NotNull Chain chain) throws IOException {
    ...
    do {
      try {
        ...
        log.trace(
            "Retry HTTP Request to {} due to status code {}",
            chain.request().url(),
            response.code());
        ...
        lastException = e;
        log.trace(
            "Retry HTTP Request to {} due to SocketTimeoutException: {}",
            chain.request().url(),
            e.getMessage());
      }
