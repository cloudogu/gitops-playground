...
# Usage: ./mirror_images_to_registry.sh REGISTRY_BASE_URL [harbor]
# if the second parameter is not nil, this script will assume that the registry is hosted by harbor
...
if [[ -n $HARBOR ]]; then
    ...
    until curl -s -o /dev/null -w "%{http_code}" $REGISTRY_BASE_URL/api/v2.0/projects | grep -q "200"; do
    ...
    skopeo copy $PROMETHEUS_OPERATOR_IMAGE --dest-creds Proxy:Proxy12345 --dest-tls-verify=false $REGISTRY_DOCKER_BASE_URL/proxy/prometheus-operator
...
skopeo copy $PROMETHEUS_OPERATOR_IMAGE --dest-creds admin:Harbor12345 --dest-tls-verify=false $REGISTRY_DOCKER_BASE_URL/library/prometheus-operator
