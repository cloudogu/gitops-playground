...
function installPlugin() {
  ...
  status=$(curl -s -o /dev/null -X POST "${JENKINS_URL}/pluginManager/installNecessaryPlugins" -u "${JENKINS_USERNAME}:${token}" -d '<jenkins><install plugin="'$1'@'$2'"/></jenkins>' -H 'Content-Type: text/xml' --write-out '%{http_code}')
  printStatus "${status}"
}
