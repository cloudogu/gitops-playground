pipeline {
    ...
    parameters {
        ...
        choice(name: 'chooseProfile', choices: ['full', 'full-netpols', 'operator-full-netpols', 'full-secrets', 'minimal', 'all-profiles', 'full-prefix', 'content-examples', 'operator-full','operator-mandants'], description: 'Starts GOP with given profile only and execute tests which belongs to profile.')
    }
                    ...
                    steps {
                        script {
                            ...
                            if (isTriggeredByTimer() || params.chooseProfile == 'all-profiles' || env.BRANCH_NAME == 'main') {
                                profiles = ['minimal', 'full', 'full-netpols', 'operator-full-netpols', 'full-secrets', 'full-prefix', 'content-examples', 'operator-full','operator-mandants']
...
"""
                            ...
                            profiles.each { profile ->
                                withK3dCluster(profile) {
                                    ...
                                    if (profile == 'full-netpols' || profile == 'operator-full-netpols') {
