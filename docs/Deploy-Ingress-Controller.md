...
In order to make use of the ingress controller, it is recommended to use it in conjunction with `--base-url`, which will
create `Ingress` objects for all components of the GitOps playground.
...
gitops-playground [configuration file](./Configuration.md).
