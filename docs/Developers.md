...
The versions are also specified in the `Config.java` file, so it is recommended to consult that file for the latest
version information.
...
The GOP can be tested with a local Keycloak realm. SCM-Manager is excluded because it currently has no OIDC support in
GOP.
