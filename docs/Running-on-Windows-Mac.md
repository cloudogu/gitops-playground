...
* In general: We cannot use the `host` network, so it's easiest to access
  via [ingress controller](./Deploy-Ingress-Controller.md).
...
* In case of problems resolving e.g. `jenkins.localhost`, you could try using `--base-url=http://local.gd` or similar,
  as described in [ingress controller](./Deploy-Ingress-Controller.md#local-ingresses).

## Mac and Windows WSL

On macOS and when using the Windows Subsystem Linux on Windows (WSL), you can just run
our [TL;DR command](../README.md#tldr) after installing Docker.
...
```
$ docker run -t -d -u 0:133 -v ... -e ******** alpine/kubectl:1.35.4 cat
...
* [Install k3d](https://k3d.io/stable/#releases), see [init-cluster.sh](../scripts/init-cluster.sh) for `K3D_VERSION`,
  e.g. using `winget`
